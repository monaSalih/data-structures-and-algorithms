...
function transformToLis(obj){
  // Solution code here...
  let storArr = [];
  let htmlToArr = Object.entries(obj);
  htmlToArr.map(item => {
    storArr.push(`<li>${item[0]}: ${item[1]}</li>`);
  });
...
let findMaleAndFemale = (data) => {
  ...
  data.map(obj => {
    if (obj.gender === 'male' || obj.gender ==='female') {
      genderArr.push(obj.name);
    }
  });
 let joinArr = genderArr.join(' and ');
  return joinArr;
};
...
let findShortest = (data) => {
  // Solution code here...
  const shortName = data.reduce((starChar , item ) => {
    if(Number(starChar.height) > Number(item.height))
    starChar = item ;
    return starChar ;
  });
