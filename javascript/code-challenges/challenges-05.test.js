...
const toLastNames = people => {
  ...
  let sortArray=[];
  people.map(index=>
  {sortArray.push(index.firstName+' '+index.lastName);
  }
  );
  return sortArray;
...
const addValues = (arr) => {
  ...
  let reduceAdd=arr.reduce((add,index)=>{
    add=add+index;
    return add;
...
const addPurchases = (arr) => {
  // Solution code here...
  let reduceAdd=arr.reduce((item,purch)=>
  {
    item=item+(purch.purchasePrice);
    return item;}
  ,0);
  return reduceAdd;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let reduceAdd2=arr.reduce((index)=>index+1,0);
  return reduceAdd2;
...
const returnNames = (arr) => {
  ...
  let reduceName=arr.reduce((acc,value)=>
  {
    acc.push(value.name);
    return acc;
  },[]);
...
const reversedString = (str) => {
  // Solution code here...
  return str.split('').reduce((inEl, element) => {
    return element + inEl;
  }, '');
